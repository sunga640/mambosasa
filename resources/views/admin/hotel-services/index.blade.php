...
@section('content')
    ...
    <p><a href="{{ route('admin.hotel-services.create') }}" class="dash-btn dash-btn--primary" style="text-decoration:none;display:inline-flex;">{{ __('Add service') }}</a></p>
            ...
            @forelse ($services as $row)
                <tr>
                    ...
                    <td class="admin-actions">
                        <a href="{{ route('admin.hotel-services.edit', $row) }}" class="dash-btn dash-btn--ghost" style="text-decoration:none;">{{ __('Edit') }}</a>
                        <form method="POST" action="{{ route('admin.hotel-services.destroy', $row) }}" style="display:inline;" onsubmit="return confirm(@json(__('Delete this service?')));">
                            @csrf
                            @method('DELETE')
