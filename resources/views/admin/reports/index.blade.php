...
@section('content')
        ...
        <div style="display:flex; flex-direction: column; align-items: flex-end; gap: 10px;">
            ...
            <div style="display:flex; gap:8px;">
                <a href="{{ route('admin.reports.export', array_merge(request()->all(), ['format' => 'csv'])) }}"
                   class="dash-btn" style="background:#f8fafc; border:1px solid #e2e8f0; color:#475569; padding:6px 12px; font-size:13px; border-radius:8px; display:flex; align-items:center; gap:6px;">
                ...
                <a href="{{ route('admin.reports.export', array_merge(request()->all(), ['format' => 'pdf'])) }}"
                   class="dash-btn" style="background:#f8fafc; border:1px solid #e2e8f0; color:#dc2626; padding:6px 12px; font-size:13px; border-radius:8px; display:flex; align-items:center; gap:6px;">
                    <i class="fa fa-file-pdf text-16"></i> {{ __('Export PDF') }}
                </a>
            </div>

            <!-- FILTER SECTION -->
            <form method="GET" action="{{ route('admin.reports.index') }}" style="display:flex; gap:10px; background:#fff; padding:8px; border-radius:12px; border:1px solid #e2e8f0;">
                <input type="date" name="from" value="{{ $from->format('Y-m-d') }}" style="border:1px solid #ddd; padding:5px 10px; border-radius:8px; font-size:13px;">
                <input type="date" name="to" value="{{ $to->format('Y-m-d') }}" style="border:1px solid #ddd; padding:5px 10px; border-radius:8px; font-size:13px;">
    ...
    <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(280px, 1fr)); gap:1.25rem; margin-bottom:2.5rem;">
        ...
        <div style="padding:1.5rem; border-radius:16px; background:#fff; border:2px solid #3b82f6; position:relative; overflow:hidden;">
            <div class="text-12" style="text-transform:uppercase; color:#64748b; font-weight:600;">{{ __('Revenue Growth (MoM)') }}</div>
            ...
            <p class="text-13 mt-5" style="color:#64748b;">{{ __('Vs Last Month:') }} TZS {{ number_format($revenueLastMonth, 0) }}</p>
        </div>

        <div style="padding:1.5rem; border-radius:16px; background:#fff; border:1px solid #e2e8f0;">
            <div class="text-12" style="text-transform:uppercase; color:#64748b; font-weight:600;">{{ __('Revenue Today') }}</div>
        ...
        <div style="padding:1.5rem; border-radius:16px; background:#0f172a; color:#fff;">
    ...
    <div style="display:grid; grid-template-columns: repeat(3, 1fr); gap: 1.5rem; margin-bottom: 2rem;">
        ...
        <!-- REPORT 3: EXPORT -->
        <a href="{{ route('admin.reports.show', ['type' => 'full']) }}" class="report-hub-card" style="text-align: center; padding: 2rem; background: #f8fafc;">
    ...
    <div style="display:grid; grid-template-columns: 2fr 1fr; gap:1.5rem; margin-bottom: 1.5rem;">
        ...
        <div style="padding:1.5rem; border:1px solid #e5e5e5; border-radius:16px; background:#fff;">
