...
@section('content')
    <h1 class="text-30">{{ __('Edit role') }}</h1>
    <form method="POST" action="{{ route('admin.roles.update', $role) }}" class="mt-30">
        @csrf
        @method('PUT')
        <div class="form-row">
        ...
        <div class="form-row">
            <label for="slug">{{ __('Slug') }} @if($role->is_system)<span class="text-13" style="opacity:.7;">({{ __('fixed for system role') }})</span>@endif</label>
            <input id="slug" type="text" name="slug" value="{{ old('slug', $role->slug) }}" @if($role->is_system) readonly style="opacity:.7;" @endif>
        ...
        @error('role')<div class="text-accent-1 text-13 mb-15">{{ $message }}</div>@enderror
