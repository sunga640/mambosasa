<x-guest-layout :title="__('Log in')">
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <!-- Password Input -->
<div class="account-field-stack mt-4">
    <x-input-label for="password" :value="__('Password')" />

    <!-- Wrapper ya kuweka input na jicho pamoja -->
    <div class="password-wrapper" style="position: relative; display: flex; align-items: center;">
        ...
        <!-- Button ya Jicho -->
        <button type="button" onclick="togglePasswordVisibility()" class="password-toggle-btn" style="position: absolute; right: 0; bottom: 8px; background: none; border: none; cursor: pointer; color: #64748b;">
    ...
    <x-input-error :messages="$errors->get('password')" class="mt-1" />
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 1.5rem;">
            <div class="remember-row" style="display: flex; align-items: center; gap: 0.5rem;">
                <input id="remember_me" type="checkbox" name="remember" style="width: 16px; height: 16px; accent-color: #122223;">
                <label for="remember_me" style="text-transform: none; color: #122223; letter-spacing: 0;">{{ __('Remember me') }}</label>
            </div>

            @if (Route::has('password.request'))
                <a class="account-link" href="{{ route('password.request') }}" style="font-size: 0.9rem; color: #64748b; text-decoration: none;">
                    {{ __('Forgot password?') }}
                </a>
            @endif
        </div>
    ...
    function togglePasswordVisibility() {
        ...
        if (passwordInput.type === 'password') {
            ...
            // Badilisha icon iwe na mstari (Slash) unapotaka kuficha
            ...
            // Rudisha icon ya jicho la kawaida
