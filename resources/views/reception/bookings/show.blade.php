...
@section('content')
    <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:1rem;">
        ...
        <a href="{{ route('reception.bookings.index') }}">{{ __('← Back to list') }}</a>
    </div>
    ...
    <div class="mt-30" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:1.25rem;">
        ...
        <div style="padding:1rem;border:1px solid #e8e8e8;border-radius:10px;background:#fafafa;">
            ...
            @endif
            @if ($booking->method?->slug === 'cash' && $booking->status === \App\Enums\BookingStatus::PendingPayment)
                <form method="POST" action="{{ route('reception.bookings.confirm-cash', $booking) }}" class="mt-15">
                    @csrf
                    <button type="submit" class="button -md -accent-1 bg-accent-1 text-white" style="padding:.5rem 1rem;border-radius:8px;border:none;cursor:pointer;width:100%;">
                        {{ __('Confirm cash payment received') }}
                    </button>
                </form>
            @endif
        </div>
    ...
    @if ($booking->invoice)
        <div class="mt-20 p-20" style="border:1px solid #e2e8f0;border-radius:10px;background:#f8fafc;">
            ...
            <div class="admin-actions mt-15">
                ...
                <form method="POST" action="{{ route('reception.invoices.resend', $booking->invoice) }}" style="display:inline;">
                    @csrf
    ...
    <div class="mt-30" style="padding:1.25rem;border:1px solid #e0e0e0;border-radius:10px;max-width:480px;">
        ...
        <form method="POST" action="{{ route('reception.bookings.update', $booking) }}">
            @csrf
            @method('PUT')
            <div class="form-row">
