...
@section('content')
    <h1 class="text-30">{{ __('Guests') }}</h1>
    <p class="text-14 mt-10" style="opacity:.8;">{{ __('Created or updated automatically when guests book.') }}</p>

    <form method="GET" action="{{ route('reception.customers.index') }}" class="form-row mt-20" style="display:flex;gap:1rem;flex-wrap:wrap;align-items:flex-end;">
        <div>
            ...
            @foreach ($customers as $c)
                <tr>
                    ...
                    <td class="admin-actions">
                        <form method="POST" action="{{ route('reception.customers.toggle', $c) }}" style="display:inline;">
                            @csrf
                        ...
                        <form method="POST" action="{{ route('reception.customers.destroy', $c) }}" style="display:inline;" data-swal-delete data-swal-title="{{ __('Delete customer?') }}">
                            @csrf
                            @method('DELETE')
