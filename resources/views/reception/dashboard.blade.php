...
@section('content')
    ...
    @isset($branchSummary)
    <div class="mb-25" style="padding:1.1rem 1.25rem;border:1px solid #e2e8f0;border-radius:12px;background:linear-gradient(135deg,#f8fafc 0%,#fff 100%);">
        <div class="text-12" style="text-transform:uppercase;letter-spacing:.06em;color:#64748b;font-weight:600;">{{ __('Branches') }}</div>
        <p class="text-15" style="margin:.35rem 0 0;">
            <span style="color:#15803d;font-weight:600;">{{ __('Active') }}: {{ number_format($branchSummary['active']) }}</span>
            <span style="margin:0 .5rem;opacity:.4;">|</span>
            <span style="color:#b45309;font-weight:600;">{{ __('Inactive') }}: {{ number_format($branchSummary['inactive']) }}</span>
        </p>
    </div>
    @endisset
    ...
    <!-- REVENUE TODAY -->
    <div style="padding:1rem 1.1rem;border-radius:10px;border:1px solid #bfdbfe;background:#eff6ff;">
        <div class="text-12" style="text-transform:uppercase;letter-spacing:.06em;color:#1e40af;font-weight:600;">{{ __('Revenue (Today)') }}</div>
        <div class="text-22 fw-600" style="margin-top:.35rem;color:#1e3a8a;">{{ number_format($kpis['revenue_today'], 0) }}</div>
    </div>

    <!-- REVENUE THIS MONTH -->
    <div style="padding:1rem 1.1rem;border-radius:10px;border:1px solid #bbf7d0;background:#f0fdf4;">
        <div class="text-12" style="text-transform:uppercase;letter-spacing:.06em;color:#15803d;font-weight:600;">{{ __('Revenue (Month)') }}</div>
        <div class="text-22 fw-600" style="margin-top:.35rem;color:#14532d;">{{ number_format($kpis['revenue_month'], 0) }}</div>
    </div>

    <!-- REVENUE THIS YEAR -->
    <div style="padding:1rem 1.1rem;border-radius:10px;border:1px solid #e5e5e5;background:#f8fafc;">
        <div class="text-12" style="text-transform:uppercase;letter-spacing:.06em;color:#475569;font-weight:600;">{{ __('Revenue (Year)') }}</div>
        <div class="text-22 fw-600" style="margin-top:.35rem;color:#0f172a;">{{ number_format($kpis['revenue_year'], 0) }}</div>
    </div>

    <!-- BOOKINGS -->
    <div style="padding:1rem 1.1rem;border-radius:10px;border:1px solid #e5e5e5;background:#fff;">
        <div class="text-12" style="text-transform:uppercase;letter-spacing:.06em;color:#64748b;font-weight:600;">{{ __('Bookings') }}</div>
        <div class="text-28 fw-600" style="margin-top:.35rem;">{{ number_format($kpis['bookings_total']) }}</div>
    </div>

    <!-- PENDING PAY -->
    <div style="padding:1rem 1.1rem;border-radius:10px;border:1px solid #fde68a;background:#fffbeb;">
        <div class="text-12" style="text-transform:uppercase;letter-spacing:.06em;color:#92400e;font-weight:600;">{{ __('Pending pay') }}</div>
        <div class="text-28 fw-600" style="margin-top:.35rem;color:#78350f;">{{ number_format($kpis['bookings_pending']) }}</div>
    </div>

    <!-- CONFIRMED -->
    <div style="padding:1rem 1.1rem;border-radius:10px;border:1px solid #bbf7d0;background:#f0fdf4;">
        <div class="text-12" style="text-transform:uppercase;letter-spacing:.06em;color:#166534;font-weight:600;">{{ __('Confirmed') }}</div>
        <div class="text-28 fw-600" style="margin-top:.35rem;color:#14532d;">{{ number_format($kpis['bookings_confirmed']) }}</div>
    </div>
    ...
    <div style="padding:1rem 1.1rem;border-radius:10px;border:1px solid #e5e5e5;background:#fff;">
        ...
        <div class="text-22 fw-600" style="margin-top:.35rem;">{{ number_format($kpis['revenue_confirmed'], 0) }}</div>
    </div>

    <!-- CASH REVENUE -->
    <div style="padding:1rem 1.1rem;border-radius:10px;border:1px solid #fef3c7;background:#fffbeb;">
        <div class="text-12" style="text-transform:uppercase;letter-spacing:.06em;color:#92400e;font-weight:600;">{{ __('Cash revenue') }}</div>
        <div class="text-22 fw-600" style="margin-top:.35rem;color:#78350f;">{{ number_format($kpis['revenue_cash'], 0) }}</div>
    </div>

    <!-- CUSTOMERS -->
    <div style="padding:1rem 1.1rem;border-radius:10px;border:1px solid #e5e5e5;background:#fff;">
        <div class="text-12" style="text-transform:uppercase;letter-spacing:.06em;color:#64748b;font-weight:600;">{{ __('Customers') }}</div>
        <div class="text-28 fw-600" style="margin-top:.35rem;">{{ number_format($kpis['customers']) }}</div>
    </div>

    <!-- ROOMS -->
    <div style="padding:1rem 1.1rem;border-radius:10px;border:1px solid #e5e5e5;background:#fff;">
        <div class="text-12" style="text-transform:uppercase;letter-spacing:.06em;color:#64748b;font-weight:600;">{{ __('Rooms') }}</div>
        <div class="text-28 fw-600" style="margin-top:.35rem;">{{ number_format($kpis['rooms']) }}</div>
    </div>

    <!-- MAINTENANCE -->
    <div style="padding:1rem 1.1rem;border-radius:10px;border:1px solid #fee2e2;background:#fef2f2;">
        ...
        <div class="text-28 fw-600" style="margin-top:.35rem;color:#7f1d1d;">{{ number_format($kpis['maintenance_open']) }}</div>
    </div>
    <div style="padding:1rem 1.1rem;border-radius:10px;border:1px solid #fecdd3;background:#fff1f2;">
        <div class="text-12" style="text-transform:uppercase;letter-spacing:.06em;color:#9f1239;font-weight:600;">{{ __('Expenses (Month)') }}</div>
        <div class="text-22 fw-600" style="margin-top:.35rem;color:#881337;">{{ number_format($kpis['expenses_month_total'] ?? 0, 0) }}</div>
    </div>
</div>

    <div class="mt-25" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:1.25rem;align-items:stretch;">
        <div style="padding:1.25rem;border:1px solid #e5e5e5;border-radius:12px;background:#fff;min-height:280px;">
