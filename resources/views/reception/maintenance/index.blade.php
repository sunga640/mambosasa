...
@section('content')
    <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:1rem;">
        ...
        <a href="{{ route('reception.maintenance.create', ['branch_id' => $filterBranchId]) }}" class="button -md -accent-1 bg-accent-1 text-white" style="text-decoration:none;padding:.5rem 1rem;border-radius:8px;">{{ __('Log maintenance') }}</a>
    </div>
            ...
            @foreach ($records as $m)
                <tr>
                    ...
                    <td class="admin-actions">
                        <a href="{{ route('reception.maintenance.edit', $m) }}">{{ __('Edit') }}</a>
                        <form action="{{ route('reception.maintenance.destroy', $m) }}" method="POST" data-swal-delete data-swal-title="{{ __('Delete record?') }}">
                            @csrf
                            @method('DELETE')
