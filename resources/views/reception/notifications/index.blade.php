            ...
            @forelse ($notifications as $n)
                <tr style="{{ $n->read_at ? '' : 'background:#fffbeb;' }}">
                    ...
                    <td>
                        ...
                        @endif
                        @if ($n->booking)
                            <div class="text-13 mt-5"><a href="{{ route('reception.bookings.show', $n->booking) }}">{{ $n->booking->public_reference }}</a></div>
                        @endif
                    </td>
                    <td class="admin-actions">
                        @if (! $n->read_at)
                            <form method="POST" action="{{ route('reception.notifications.read', $n) }}" style="display:inline;">
                                @csrf
                        ...
                        @if ($n->kind === 'stay_ended_room_available' && $n->booking && ! $n->resolved_at)
                            <form method="POST" action="{{ route('reception.notifications.signout', $n) }}" style="display:inline;margin-left:.35rem;">
                                @csrf
